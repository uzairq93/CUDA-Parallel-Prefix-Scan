...
#define NUM_BANKS 32 // Originally 64
#define LOG_NUM_BANKS 5 // Originally 5
...
__global__ void Reduction_Kernel(float *inArray, float *resArray, int n, int offset)
{
    ...
    __shared__ float temp[BLOCK_SIZE + CONFLICT_FREE_OFFSET(BLOCK_SIZE)];
    int localId = threadIdx.x;
    // CHANGED
    int offsetLocalId = localId + CONFLICT_FREE_OFFSET(localId);
    ...
    {
        if (offset > 0)
            temp[offsetLocalId] = resArray[globalId + offset];
        else
            temp[offsetLocalId] = inArray[globalId];
    }
    else temp[offsetLocalId] = 0.0f;
    ...
    if (localId == 0)
    {
        ...
        resArray[offset + roundUpDev(n, BLOCK_SIZE) + blockIdx.x] = temp[BLOCK_SIZE + CONFLICT_FREE_OFFSET(BLOCK_SIZE - 1) - 1];
        // CHANGED
        temp[BLOCK_SIZE + CONFLICT_FREE_OFFSET(BLOCK_SIZE - 1) - 1] = 0; // CHANGED Clear the last element for exclusive scan
    }
    ...
    __syncthreads();
    resArray[globalId + offset] = temp[offsetLocalId];
}
...
__global__ void FullScan_Kernel(float *inArray, int offset)
{
    ...
    __shared__ float temp[BLOCK_SIZE + CONFLICT_FREE_OFFSET(BLOCK_SIZE)]; // Create enough shared mem to contain the block
    
    // changed
    int ogtid = threadIdx.x; // Local/Global ID of this thread since this is the only block
    int tid = CONFLICT_FREE_OFFSET(ogtid) + ogtid;
    temp[tid] = inArray[offset + ogtid];
    ...
    for (int d = BLOCK_SIZE >> 1; d > 0; d >>= 1)
    {
        ...
        if (ogtid < d)
        {
            int left = stride * (2 * ogtid + 1) - 1;
            int right = stride * (2 * ogtid + 2) - 1;
            // CHANGED
            left += CONFLICT_FREE_OFFSET(left);
            right += CONFLICT_FREE_OFFSET(right);
            temp[right] += temp[left]; // TODO: Can we reduce bank conflicts here?
    ...
    if (ogtid == 0)
        temp[BLOCK_SIZE + CONFLICT_FREE_OFFSET(BLOCK_SIZE - 1) - 1] = 0; // Clear the last element for exclusive scan
...
__global__ void FullScan_Kernel_InToOut(float *inArray, float *outArray, int n)
{
    ...
    // CHANGED
    int ogtid = threadIdx.x; // Local/Global ID of this thread since this is the only block
    // int tid = CONFLICT_FREE_OFFSET(ogtid);
    int tid = CONFLICT_FREE_OFFSET(ogtid) + ogtid;

    if (ogtid < n) temp[tid] = inArray[ogtid];
    else temp[tid] = 0.0f;
    ...
    /* STEP 3: Clear the last element for an exclusive scan */
    if (ogtid == 0) temp[BLOCK_SIZE + CONFLICT_FREE_OFFSET(BLOCK_SIZE - 1) - 1] = 0; // Clear the last element for exclusive scan
    ...
    __syncthreads();
    // CHANGED
    if (ogtid < n) outArray[ogtid] = temp[tid];
}
...
__global__ void AddOffset_Kernel(float *outArray, float *resArray, int n, int offset)
{
    ...
    */
    int globalId = blockIdx.x * BLOCK_SIZE + threadIdx.x;
    if (outArray == NULL)
        resArray[globalId + offset] += resArray[offset + n + blockIdx.x];
    else if (globalId < n)
        outArray[globalId] = resArray[globalId] + resArray[roundUpDev(n, BLOCK_SIZE) + blockIdx.x];
}
