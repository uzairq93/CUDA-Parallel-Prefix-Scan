...
#define DEFAULT_NUM_ELEMENTS 16777216 // This was originally 16000000
#define MAX_RAND 3
...
//                h_data[i] = 1.0f;
    ...
    /*
    // Display inputs (delete this later)
    printf("Input array: \n");
    ...
    */
    ...
    // Initialize intermediate arrays on device
    float* intermediates = allocateIntermediatesOnDevice(num_elements);

    // **===-----------------------------------------------------------===**

    // Run just once to remove startup overhead for more accurate performance 
    // measurement
    prescanArray(d_odata, d_idata, intermediates, 16); //UNCOMMENT THIS
    ...
    prescanArray(d_odata, d_idata, intermediates, num_elements);
    // **===-----------------------------------------------------------===**
    ...
    CUDA_SAFE_CALL(cudaMemcpy( h_data, d_odata, mem_size, cudaMemcpyDeviceToHost));

    /*
    // Display outputs (delete this later)
